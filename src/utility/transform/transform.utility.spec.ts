...
describe('Rocket Transform Utility:', () => {
   ...
   it('Should transform bytes into a readable string.', () => {
      ...
      expect(RocketTransform.bytes({bytes: 3200000000000000})).toEqual('3.2 PB');
      expect(RocketTransform.bytes({bytes: 6440000000000000000})).toEqual('6.44 EB');
      expect(RocketTransform.bytes({bytes: 88700000000000000000000})).toEqual('88.7 ZB');
      expect(RocketTransform.bytes({bytes: 46200000000000000000000000})).toEqual('46.2 YB');
   });
});
